#' Performs matching of schr_tbl_chars() objects
#'
...
#'
schr_match_combinations <- function(match_this,
                                    against_these,
  ...
  filtered_df <-
    against_these %>%
    dplyr::mutate(locn = dplyr::row_number(), .before = 1) %>%
  ...
  filtered_locns <- filtered_df %>% dplyr::select(locn)


  # create a tibble of repeated "match_this" rows to mimics the rowcount of filtered_df
  empty_df <- filtered_df %>% dplyr::bind_rows(match_this) %>% dplyr::slice(0)

  filtered_df <- dplyr::bind_rows(empty_df,filtered_df) %>%
  ...
  match_this <- dplyr::bind_rows(empty_df,match_this) %>%
    dplyr::select(-locn,-char_count) %>%
    dplyr::mutate(dplyr::across(.fns = ~as.integer(tidyr::replace_na(.,0)))) %>%
    dplyr::bind_rows(base::replicate(nrow(filtered_df)-1, ., simplify = FALSE))
  ...
  dist_calcs_pos <-
    dist_calcs %>%
    dplyr::mutate(dplyr::across(.fns = ~dplyr::if_else(.>0, ., as.integer(0)))) %>%
    rowSums()
  dist_calcs_neg <-
    dist_calcs %>%
    dplyr::mutate(dplyr::across(.fns = ~dplyr::if_else(.<0, ., as.integer(0)))) %>%
  ...
  a_b_distances %>%
    dplyr::filter(a_not_in_b <= a_not_in_b_tolerance,
           b_not_in_a <= b_not_in_a_tolerance) %>%
    dplyr::arrange(sum(a_not_in_b, b_not_in_a), a_not_in_b, b_not_in_a)
